...
with col1:
    Credit_Mix = st.selectbox(
        label='Credit_Mix', options=encoder_Credit_Mix.classes_, index=1)
    data["Credit_Mix"] = [Credit_Mix]

with col2:
    Payment_of_Min_Amount = st.selectbox(
        label='Payment_of_Min_Amount', options=encoder_Payment_of_Min_Amount.classes_, index=1)
    data["Payment_of_Min_Amount"] = [Payment_of_Min_Amount]

with col3:
    Payment_Behaviour = st.selectbox(
        label='Payment_Behaviour', options=encoder_Payment_Behaviour.classes_, index=5)
...
with col2:
    Num_Bank_Accounts = int(st.number_input(
        label='Num_Bank_Accounts', value=3))
...
with col2:
    # st.header("Kolom 1")
    Delay_from_due_date = int(st.number_input(
        label='Delay_from_due_date', value=3))
    data["Delay_from_due_date"] = Delay_from_due_date

with col3:
    Num_of_Delayed_Payment = int(st.number_input(
        label='Num_of_Delayed_Payment', value=7))
    data["Num_of_Delayed_Payment"] = Num_of_Delayed_Payment

with col4:
    Changed_Credit_Limit = float(st.number_input(
        label='Changed_Credit_Limit', value=11.27))
    data["Changed_Credit_Limit"] = Changed_Credit_Limit

col1, col2, col3, col4 = st.columns(4)

with col1:
    Num_Credit_Inquiries = float(st.number_input(
        label='Num_Credit_Inquiries', value=5))
    data["Num_Credit_Inquiries"] = Num_Credit_Inquiries

with col2:
    Outstanding_Debt = float(st.number_input(
        label='Outstanding_Debt', value=809.98))
    data["Outstanding_Debt"] = Outstanding_Debt

with col3:
    Monthly_Inhand_Salary = float(st.number_input(
        label='Monthly_Inhand_Salary', value=1824.8))
    data["Monthly_Inhand_Salary"] = Monthly_Inhand_Salary

with col4:
    Monthly_Balance = float(st.number_input(
        label='Monthly_Balance', value=186.26))
    data["Monthly_Balance"] = Monthly_Balance

col1, col2, col3 = st.columns(3)

with col1:
    Amount_invested_monthly = float(st.number_input(
        label='Amount_invested_monthly', value=236.64))
    data["Amount_invested_monthly"] = Amount_invested_monthly

with col2:
    Total_EMI_per_month = float(st.number_input(
        label='Total_EMI_per_month', value=49.5))
    data["Total_EMI_per_month"] = Total_EMI_per_month

with col3:
    Credit_History_Age = float(st.number_input(
        label='Credit_History_Age', value=216))
